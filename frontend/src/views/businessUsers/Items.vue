<template>
  <div>
      ...
      <div class="container-fluid d-flex align-items-center">
        <div class="row">
          <div class="col-lg-7 col-md-10">
            <p class="text-white mt-0 mb-5">{{ $t("businessItemsPage.secondaryHeader") }}</p>
          </div>
        </div>
      </div>
    </base-header>
    ...
    <modal
      ...
      :showClose="false"
    >
      <div slot="header" class="modal-title">{{ $t("businessItemsPage.modal.header") }}</div>
      <card
        type="secondary"
      ...
      <template slot="footer">
        <base-button type="link" @click="showAddingModal = false">{{ $t("common.closeBtn") }}</base-button>
        <base-button type="success" class="ml-auto" @click="addItemClick">{{ $t("common.addBtn") }}</base-button>
      </template>
    </modal>
