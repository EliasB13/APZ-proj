    ...
    <template v-slot:header>
      <b-row>
        ...
        <b-col cols="auto" align-self="center">
          <base-button v-if="itemsSelectionMode" type="link" @click="resetItemsClick">Reset</base-button>
          <base-button
          ...
          ></base-button>
          <base-button type="danger" size="sm" icon="fas fa-minus" @click="removeItemClick"></base-button>
        </b-col>
      </b-row>
    </template>
              ...
              <div @click="modalItemClick(mItem.id)">
                <b-row align-v="center">
                  ...
                  <b-col>
                    ...
                    <div class="h5 font-weight-300 mb-1">{{ mItem.description }}</div>
                  </b-col>
                </b-row>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      items: state => state.readers.items,
      status: state => state.readers.status
    }),
    showSpinner() {
      return (
        this.status.readerItemsLoading ||
  ...
  watch: {
    ...
    showItems(newV, oldV) {
      if (newV == true) {
        if (!this.status.itemsLoaded) this.getReaderItems(this.readerId);
  ...
  methods: {
    ...
    addItemClick() {
      this.modalItemsCopy.forEach(i => {
        if (i.selected)
          this.addItemToReader({ itemId: i.id, readerId: this.readerId });
      });
    ...
    removeItemClick() {
      if (!this.itemsSelectionMode) this.itemsSelectionMode = true;
        ...
        this.itemsCopy.forEach(i => {
          if (i.selected) {
            this.removeItemFromReader(i.id);
          }
        });
    },
    resetItemsClick() {
